...
class MS_SSIM_L1Loss(nn.Module):
    def __init__(self, gaussian_sigmas=[0.5, 1.0, 2.0, 4.0, 8.0],
        ...
        self.register_buffer('g_masks', g_masks)  # Register g_masks as a buffer
    ...
    def forward(self, x, y, data_range=1.0):
        ...
        gaussian_l1 = conv2d(loss_l1, self.g_masks.narrow(dim=0, start=-1, length=1), padding=self.pad).mean(1)

        loss_mix = self.alpha * loss_ms_ssim + (1 - self.alpha) * gaussian_l1 / data_range
